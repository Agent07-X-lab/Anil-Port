...
'use client'
import React, { useState, Children, useRef, useLayoutEffect, FC, ReactNode, HTMLAttributes } from 'react';
...
const Stepper: FC<StepperProps> = ({
  ...
  return (
    <div className="outer-container" {...rest}>
      <div className={`step-circle-container glass-card ${stepCircleContainerClassName}`}>
          ...
          <div className={`footer-container ${footerClassName}`}>
            <div className={`footer-nav ${currentStep !== 1 ? 'spread' : 'end'}`}>
              {currentStep !== 1 && (
               ...
               {!isLastStep && <button onClick={isLastStep ? handleComplete : handleNext} className="next-button" {...nextButtonProps}>
  ...
  function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators }: StepIndicatorProps) {
    ...
    return (
      <motion.div onClick={handleClick} className="step-indicator" animate={status} initial={false}>
        <motion.div
          variants={{
            inactive: { scale: 1, backgroundColor: 'hsl(var(--input))', color: 'hsl(var(--muted-foreground))' },
